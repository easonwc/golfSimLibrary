import {
  assertFiniteNumber,
  assertNonEmptyString,
  ValidationError,
} from "../../errors.js";
import type { Golfer, Hole, ShortGameLie } from "../../types/index.js";
import type { ShortGameInput } from "./types.js";

const DEFAULT_TRIALS = 5_000;
const MIN_TRIALS = 100;
const MAX_TRIALS = 100_000;

const VALID_LIES: ReadonlySet<ShortGameLie> = new Set([
  "fringe",
  "rough",
  "bunker",
  "deepRough",
]);

function validateAbility(value: unknown, field: string): number {
  return assertFiniteNumber(value, field, { min: 0, max: 100 });
}

function validateGolfer(golfer: unknown): Golfer {
  if (golfer === null || typeof golfer !== "object") {
    throw new ValidationError("golfer must be an object");
  }

  const g = golfer as Record<string, unknown>;
  const id = assertNonEmptyString(g.id, "golfer.id");

  if (g.shortGame === null || typeof g.shortGame !== "object") {
    throw new ValidationError("golfer.shortGame must be an object");
  }

  const shortGame = g.shortGame as Record<string, unknown>;

  return {
    id,
    name: typeof g.name === "string" ? g.name : undefined,
    shortGame: {
      shortGame: validateAbility(
        shortGame.shortGame,
        "golfer.shortGame.shortGame",
      ),
      chipping: validateAbility(
        shortGame.chipping,
        "golfer.shortGame.chipping",
      ),
      bunkerPlay: validateAbility(
        shortGame.bunkerPlay,
        "golfer.shortGame.bunkerPlay",
      ),
      pitching: validateAbility(
        shortGame.pitching,
        "golfer.shortGame.pitching",
      ),
    },
  };
}

function validateHole(hole: unknown): Hole {
  if (hole === null || typeof hole !== "object") {
    throw new ValidationError("hole must be an object");
  }

  const h = hole as Record<string, unknown>;
  const id = assertNonEmptyString(h.id, "hole.id");
  const par = h.par;

  if (par !== 3 && par !== 4 && par !== 5) {
    throw new ValidationError("hole.par must be 3, 4, or 5");
  }

  const lengthYards = assertFiniteNumber(h.lengthYards, "hole.lengthYards", {
    min: 50,
    max: 750,
  });

  if (h.green === null || typeof h.green !== "object") {
    throw new ValidationError("hole.green must be an object");
  }

  const green = h.green as Record<string, unknown>;

  if (h.shortGame === null || typeof h.shortGame !== "object") {
    throw new ValidationError("hole.shortGame must be an object");
  }

  const shortGame = h.shortGame as Record<string, unknown>;

  return {
    id,
    number:
      typeof h.number === "number" && Number.isInteger(h.number)
        ? h.number
        : undefined,
    par,
    lengthYards,
    green: {
      sizeSqFt: assertFiniteNumber(green.sizeSqFt, "hole.green.sizeSqFt", {
        min: 1_000,
        max: 12_000,
      }),
      speed: assertFiniteNumber(green.speed, "hole.green.speed", {
        min: 6,
        max: 16,
      }),
      slope: assertFiniteNumber(green.slope, "hole.green.slope", {
        min: 0,
        max: 1,
      }),
      pinDifficulty: assertFiniteNumber(
        green.pinDifficulty,
        "hole.green.pinDifficulty",
        { min: 0, max: 1 },
      ),
    },
    shortGame: {
      roughDifficulty: assertFiniteNumber(
        shortGame.roughDifficulty,
        "hole.shortGame.roughDifficulty",
        { min: 0, max: 1 },
      ),
      bunkerDifficulty: assertFiniteNumber(
        shortGame.bunkerDifficulty,
        "hole.shortGame.bunkerDifficulty",
        { min: 0, max: 1 },
      ),
      collectionDifficulty: assertFiniteNumber(
        shortGame.collectionDifficulty,
        "hole.shortGame.collectionDifficulty",
        { min: 0, max: 1 },
      ),
    },
  };
}

function validateLie(value: unknown): ShortGameLie {
  if (typeof value !== "string" || !VALID_LIES.has(value as ShortGameLie)) {
    throw new ValidationError(
      "lie must be one of: fringe, rough, bunker, deepRough",
    );
  }
  return value as ShortGameLie;
}

export function validateShortGameInput(input: unknown): ShortGameInput {
  if (input === null || typeof input !== "object") {
    throw new ValidationError("input must be an object");
  }

  const raw = input as Record<string, unknown>;
  const golfer = validateGolfer(raw.golfer);
  const hole = validateHole(raw.hole);
  const lie = validateLie(raw.lie);

  const missDistanceYards = assertFiniteNumber(
    raw.missDistanceYards,
    "missDistanceYards",
    { min: 3, max: 80 },
  );

  let trials = DEFAULT_TRIALS;
  if (raw.trials !== undefined) {
    trials = assertFiniteNumber(raw.trials, "trials", {
      min: MIN_TRIALS,
      max: MAX_TRIALS,
    });
    if (!Number.isInteger(trials)) {
      throw new ValidationError("trials must be an integer");
    }
  }

  let seed: number | undefined;
  if (raw.seed !== undefined) {
    seed = assertFiniteNumber(raw.seed, "seed");
    if (!Number.isInteger(seed)) {
      throw new ValidationError("seed must be an integer");
    }
  }

  return {
    golfer,
    hole,
    missDistanceYards,
    lie,
    trials,
    seed,
  };
}

export { DEFAULT_TRIALS, MIN_TRIALS, MAX_TRIALS };
