...
import type { PuttingInput } from "./types.js";
...
function validateGolfer(golfer: unknown): Golfer {
  ...
  if (g.putting === null || typeof g.putting !== "object") {
    throw new ValidationError("golfer.putting must be an object");
  }

  const putting = g.putting as Record<string, unknown>;

  return {
    id,
    name: typeof g.name === "string" ? g.name : undefined,
    putting: {
      putting: validateAbility(putting.putting, "golfer.putting.putting"),
      shortPutting: validateAbility(
        putting.shortPutting,
        "golfer.putting.shortPutting",
      ),
      lagPutting: validateAbility(
        putting.lagPutting,
        "golfer.putting.lagPutting",
      ),
    },
  };
}
...
export function validatePuttingInput(input: unknown): PuttingInput {
  ...
  if (raw.firstPuttDistanceFeet !== undefined) {
    firstPuttDistanceFeet = assertFiniteNumber(
      raw.firstPuttDistanceFeet,
